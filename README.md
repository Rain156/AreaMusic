...
1. 将 `areamusic-neoforge-1.21.1-0.0.1.jar` 放入服务端和每个客户端的 `mods/`。
...
Linux/macOS 使用 `./gradlew clean build`。发布 JAR 位于 `build/libs/areamusic-neoforge-1.21.1-0.0.1.jar`。
...
Install the mod on the server and every client, and keep matching MusicIDs under lowercase `<gameDir>/areamusic`. Audio bytes are never sent over the network. This branch targets Minecraft 1.21.1, NeoForge 21.1.235, and Java 21.
